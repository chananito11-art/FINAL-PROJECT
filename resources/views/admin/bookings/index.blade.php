...
.filter-tab{padding:7px 16px;border-radius:999px;font-size:.82rem;font-weight:600;text-decoration:none;color:rgba(240,242,255,.55);background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);transition:all .2s}
...
<div class="card">
    <div class="card-header">
        ...
        <span style="font-size:.85rem;color:rgba(240,242,255,.45)">{{ $bookings->total() }} total</span>
    </div>
    <div class="tw">
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <div style="font-size:.8rem;color:rgba(240,242,255,.45)">{{ $b->email }}</div>
                </td>
            ...
            @empty
            <tr><td colspan="7" style="text-align:center;padding:40px;color:rgba(240,242,255,.4)">No bookings found.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
    @if($bookings->hasPages())
    <div style="padding:16px;border-top:1px solid rgba(255,255,255,.06)">{{ $bookings->links() }}</div>
    @endif
</div>
@endsection
