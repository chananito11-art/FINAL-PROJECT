...
<div class="card">
    <div class="card-header"><span class="card-title">All Users</span><span style="font-size:.85rem;color:rgba(240,242,255,.45)">{{ $users->total() }} total</span></div>
    <div class="tw">
        <table>
            ...
            <tr>
                ...
                <td style="font-size:.88rem;color:rgba(240,242,255,.7)">{{ $user->email }}</td>
                <td style="font-size:.85rem;color:rgba(240,242,255,.5)">{{ $user->phone ?? '—' }}</td>
                <td>
                ...
                <td style="font-size:.82rem;color:rgba(240,242,255,.45)">{{ $user->created_at->format('M d, Y') }}</td>
                <td>
            ...
            @empty
            <tr><td colspan="6" style="text-align:center;padding:40px;color:rgba(240,242,255,.4)">No users found.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
...
<div id="roleModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:200;align-items:center;justify-content:center">
    <div style="background:#0d1128;border:1px solid rgba(255,255,255,.1);border-radius:18px;width:100%;max-width:380px;padding:28px">
        <h2 style="font-size:1.05rem;font-weight:800;margin-bottom:16px">Change Role</h2>
